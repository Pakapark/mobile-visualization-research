...
def warmupGenerator():
    ...
    def writeHelper(writerhf, dateList, student, s1, s2, s3, start):
        for i, date in enumerate(dateList):
            writerhf.writerow([start, student, date, random.randint(s1, s1+15), random.randint(s2, s2+15), random.randint(s3, s3+15)])
    ...
    with open("processed/warmup.csv", "w") as f:
        ...
        writer.writerow(["id", "student", "date", "math score", "science score", "history score"])
        writeHelper(writer, date, "student A", 40, 60, 80, 0)
